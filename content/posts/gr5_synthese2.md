...
```{=html}
<p class="alinea">
Toutes les formules étudiées précédemment ont été reportées dans un excel (cf annexe) afin de résoudre le système d'équation et d'obtenir un débit d'eau douce en fonction des paramètres rentrés. Parmi ces paramètres, nous en avons selectionné quelques uns qui dépendent des conditions extérieures ou propres au système. 
```
